...
#define LADD (4)
  ...
  for(int lat=LMIN;lat<=LMAX;lat+=LADD){
      ...
      GridCartesian     Grid(latt_size,simd_layout,mpi_layout);
      std::cout<<"simd_layout"<<simd_layout<<std::endl;

      GridParallelRNG          pRNG(&Grid);      pRNG.SeedFixedIntegers(std::vector<int>({45,12,81,9}));

      LatticeComplex z(&Grid); random(pRNG,z);
      LatticeComplex x(&Grid); random(pRNG,x);
      LatticeComplex y(&Grid); random(pRNG,y);
      
#if DEBUG
      LatticeComplex err(&Grid);
      
      LatticeComplex zref(&Grid); random(pRNG,zref);
      auto xv = x.View();
      auto yv = y.View();
      auto zv = zref.View();

      for(int64_t s=0; s < zv.size(); s++) {
	zv[s]=xv[s]+yv[s];
//	zv[s]=xv[s]; 
     }
#endif 
	...
	z=x+y;
        ...
        for(int64_t s=0; s<zv.size(); s++) {
        std::cout<<"Expected: "<<zv[s]<<" Got:"<<zres[s]<<std::endl;
      }
      //}
#endif
//	z=x;
...
//	z=x;
	...
	std::cout<<"Expected: "<<zv[s]<<" Got:"<<zres[s]<<std::endl;
      ...
      double bytes=3*vol*sizeof(vTComplex);
      double flops=vol;
